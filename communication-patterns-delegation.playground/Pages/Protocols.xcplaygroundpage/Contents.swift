...
protocol Vehicle {
    ...
    // since I only have get for the protocal, you can't change the instance of the class that conforms to the protocal
}

class Car: Vehicle {
    // need to init and declare the var in protocol
    let color: String
    ...
    init(maxSpeed: Int, wheelsNum: Int, doors: Int, model: String, color: String){
...
let firstCar = Car(maxSpeed: 78, wheelsNum: 4, doors: 4, model: "sadan", color: "blue")
...
func compare(artist: Artist){
    if monet == artist {
        print("yay \(artist.name) is equal to monet")
    } else {
        print("not equal")
    }
